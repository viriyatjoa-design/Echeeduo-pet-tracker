 ...
 *
 * Actions return result objects instead of throwing: in production Next.js
 * strips a thrown Error's message from Server Actions, so the mirror pattern
 * (see lib/actions/ai.ts) surfaces the real message to the caller.
 */

/** Actions with no success payload. */
export type CareResult = { ok: true } | { ok: false; error: string };

/** completeCareEvent / logPastCareEvent — carries the chained next due date. */
export type CareCompleteResult =
  | { ok: true; nextDueDate: string | null }
...
export async function createCareEvent(
  ...
  try {
    ...
    if (!input.event_type_id)
      return { ok: false, error: "Please pick an event type." };
 ...
 */
export async function removeCareEvent(id: string): Promise<CareResult> {
  try {
    const me = await getCurrentAppUser();
    if (!me) return { ok: false, error: "Unauthorized" };
 ...
 */
export async function logPastCareEvent(
  input: LogPastCareEventInput,
): Promise<CareCompleteResult> {
  try {
    const me = await getCurrentAppUser();
    if (!me) return { ok: false, error: "Unauthorized" };
    ...
    if (!doneDate || !/^\d{4}-\d{2}-\d{2}$/.test(doneDate)) {
      return { ok: false, error: "Please pick the date it was done." };
    }
    if (doneDate > todayInTz()) {
      return {
        ok: false,
        error: "That date is in the future — use a regular event instead.",
      };
    }
    ...
    if (error) return { ok: false, error: careInsertError(error, !!consume).message };
    ...
    if (interval != null) {
      ...
      if (insErr) return { ok: false, error: careInsertError(insErr, !!consume).message };
    }
...
export async function completeCareEvent(
  ...
  try {
    ...
    if (!closed || closed.length === 0) {
      ...
      revalidate();
      return { ok: true, nextDueDate: null };
    }
...
export async function rescheduleCareEvent(
  ...
  try {
    ...
    if (!date) return { ok: false, error: "Pick a new due date." };
 ...
 */
export async function createMedCourse(
  input: CreateMedCourseInput,
): Promise<CareResult> {
  try {
    const me = await getCurrentAppUser();
    if (!me) return { ok: false, error: "Unauthorized" };
    ...
    if (!start) return { ok: false, error: "Pick a start date." };

    const duration = posIntOrNull(input.duration_days);
    if (!duration) return { ok: false, error: "Duration must be at least 1 day." };

    const times = (input.times ?? []).map((t) => clean(t)).filter(Boolean) as string[];
    if (times.length === 0) return { ok: false, error: "Add at least one dose time." };
    ...
    if (!medType)
      return { ok: false, error: "The 'medicine' care type is missing from lookups." };
