...
const config: Config = {
  ...
  theme: {
    ...
    extend: {
      ...
      // Every token carries `<alpha-value>` so Tailwind opacity modifiers
      // (bg-muted/60, text-muted-foreground/70, border-primary/40 …) actually
      // apply. Without it the modifier silently no-ops to full opacity — a bug
      // class we hit repeatedly. Bare classes still resolve at alpha 1.
      colors: {
