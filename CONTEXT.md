...
| 07-12 | **Server actions use the {ok,error} result pattern** (not throws) app-wide — Next.js strips thrown Server Action messages in prod. Value actions converted; shared ActionButton accepts throwing OR result actions. When adding a NEW mutation, return a result object + show res.error at the call-site (mirror lib/actions/ai.ts). | Prod error-masking sweep |
| 07-12 | **All tailwind color tokens carry `<alpha-value>`** so opacity modifiers (bg-x/60) work — do NOT go back to bare `hsl(var(--x))` token defs. Tests live in `tests/` (vitest, `npm test`); keep pure domain logic covered. | Recurring alpha no-op bug; test suite added |
| 07-12 | **British Shorthair care numbers** live in `lib/cat-care-facts.ts` (one source: /care-guide page + AI prompt context). Update there, not inline. | BSH handbook |
...
| 07-13 | **Feeding station** (owner picked "filling cat bowl" over a progress bar, then paired it with a matching water bowl): Today cat cards now show a matched food+water bowl pair (`components/feed/feeding-bowls.tsx`, pure SVG). Food = kibble mound in the cat's coat accent, heaps over the rim + amber treat when over target; water = blue wavy fill. **The bowls ARE the log buttons** (tap food→FeedDialog, water→water dialog) — this REPLACED the 07-12 avatar kcal-ring + thin KcalBar + Feed-button/water-pill layout. Water goal = weight-derived ≈50 ml/kg (`BREED_REF.waterMlPerKgPerDay`). Tone stays encouraging: "ml · full ✓", "over target — treat day 🐾", never scolding. New tokens `--water`/`--water-lite`/`--bowl-shadow-base`. Big dialog bowls = agreed but not yet built | Owner: "track drink water like food… cute way other than a boring bar" → filling bowl; "cute way to pair the food tracker" → matching food bowl; "lets build the real thing" |
